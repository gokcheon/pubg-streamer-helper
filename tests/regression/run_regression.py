...
def load_matcher_and_helpers():
    ...
    class_source = score_func_source = const_source = rank_func_source = None
    for node in ast.walk(tree):
        ...
        elif isinstance(node, ast.FunctionDef) and node.name == "_detect_rank_state":
            # fix_a28: 치킨 감지 함수도 추출 (메서드라 들여쓰기 제거 후 일반 함수로)
            import textwrap
            rank_func_source = textwrap.dedent(ast.get_source_segment(source, node))
...
def run_ocr_one(img_path, matcher, score_fn, score_threshold, modes_cfg, debug_dir, rank_fn=None):
    """한 장 처리 결과 반환:
    {
    ...
    # fix_a28: 치킨(#1) 판정도 함께 기록 (True/False/None)
    chicken = None
    if rank_fn is not None:
        class _S:
            error_logger = _SilentLogger()
        try:
            chicken, _ = rank_fn(_S(), img)
        except Exception:
            chicken = "error"
...
# 결과 vs 정답 비교
# ----------------------------------------------------------------
def compare_to_truth(result, expected_slots, expected_gated, expected_chicken="__skip__"):
    """반환: (status, details)
    status: 'PASS' | 'FAIL'
    ...
    # fix_a28: 치킨 판정 비교 (truth.json 의 chicken 항목이 있을 때만)
    if expected_chicken != "__skip__" and result.get("chicken") != expected_chicken:
        mismatches.append(f"치킨: 정답={expected_chicken} 실제={result.get('chicken')}")
    if mismatches:
...
def main():
    ...
    DigitTemplateMatcher, score_result_screen, score_threshold, detect_rank_state = load_matcher_and_helpers()
    ...
    for i, f in enumerate(all_files, 1):
        if not f.suffix.lower() in (".png", ".jpg", ".jpeg"):
            continue
        result = run_ocr_one(f, matcher, score_result_screen, score_threshold, modes_cfg, debug_dir, detect_rank_state)
        chicken_map = truth_data.get("chicken", {})
        exp_chicken = chicken_map.get(f.name, "__skip__") if chicken_map else "__skip__"
        if f.name in invalid_set:
            status, detail = compare_to_truth(result, None, expected_gated=True)
        elif f.name in truth_map:
            status, detail = compare_to_truth(result, truth_map[f.name], expected_gated=False, expected_chicken=exp_chicken)
        else:
