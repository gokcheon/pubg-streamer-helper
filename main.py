...
def get_data_dir() -> Path:
    """fix_a29: 사용자 데이터(config/토큰/기록/로그/캡처)가 사는 폴더.
    - 설치판(frozen): %LOCALAPPDATA%\PUBG Streamer Helper  — Program Files 에 쓰지 않으므로 관리자 권한이 필요 없다.
      (예전엔 exe 옆에 써서 바로가기에 '관리자 권한으로 실행' 을 걸어야 했고, 관리자 프로세스에서는 브라우저 열기가 불안정했으며
       토큰·설정이 Program Files 에 남았다)
    - 폴더판(python main.py): 예전처럼 앱 폴더 그대로 (곡천님 개발 흐름 유지).
    - 환경변수 PUBG_HELPER_DATA_DIR 로 강제 지정 가능 (테스트용).
    """
    override = os.environ.get("PUBG_HELPER_DATA_DIR", "").strip()
    if override:
        return Path(override)
    if getattr(sys, "frozen", False):
        base = os.environ.get("LOCALAPPDATA", "").strip()
        if base:
            return Path(base) / "PUBG Streamer Helper"
        return Path.home() / "AppData" / "Local" / "PUBG Streamer Helper"
    return RUNTIME_BASE_DIR


DATA_DIR = get_data_dir()
CONFIG_PATH = DATA_DIR / "config.json"
OVERLAY_PATH = DATA_DIR / "overlay.txt"
OUTBOX_PATH = DATA_DIR / "chat_outbox.txt"
TOKENS_PATH = DATA_DIR / "chat_tokens.json"
LOGS_DIR = DATA_DIR / "logs"
# CAPTURES_DIR 제거: screenshot 폴더로 통일
SCREENSHOTS_DIR = DATA_DIR / "screenshot"
...
class HotkeyManager:
    def __init__(self, callback, event_logger: logging.Logger, error_logger: logging.Logger, console_logger: logging.Logger,
        ...
        # fix_a28: F8 과 F9 를 따로 켤 수 있게 (스팀 모드 = F8 끄고 F9 만 유지)
        self.enable_f8 = True
        self.enable_f9 = True
        self._lifecycle_lock = threading.Lock()

    def start(self, enable_f8: bool = True, enable_f9: bool = True) -> None:
        with self._lifecycle_lock:
            # fix_a28: 이미 돌고 있으면 먼저 완전히 내리고(스레드 join) 다시 올린다.
            # 예전엔 stop() 직후 start() 하면 이전 스레드가 UnregisterHotKey 하기 전에
            # 새 스레드가 RegisterHotKey 를 시도해 code=1409 로 실패가 고정됐다.
            if self.is_running:
                self._stop_locked()
            if not enable_f8 and not enable_f9:
                return
            self.enable_f8 = bool(enable_f8)
            self.enable_f9 = bool(enable_f9)
    ...
    def _run(self) -> None:
        ...
        f9_registered = False
        if self.enable_f8:
            if user32.RegisterHotKey(None, HOTKEY_ID_F8, MOD_NOREPEAT, VK_F8):
                f8_registered = True
                self.event_logger.info("F8 전역 핫키 등록 완료")
                self.console_logger.info("F8 전역 핫키 등록 완료")
            else:
                err = ctypes.windll.kernel32.GetLastError()
                # fix_a15.3: 사용자가 알 수 있도록 상태 노출 (F9 는 계속 시도)
        ...
        if self.enable_f9 and self.send_callback is not None:
            if user32.RegisterHotKey(None, HOTKEY_ID_F9, MOD_NOREPEAT, VK_F9):
                ...
                self.event_logger.warning(
                    "F9 핫키 등록 실패 (다른 프로그램 점유 추정, code=%d)", err
                )

        if not f8_registered and not f9_registered:
            self.is_running = False
            return

        msg = ctypes.wintypes.MSG()
        # GetMessageW 는 WM_QUIT 수신 시 0 을 반환해 루프가 끝난다 (별도 분기 불필요)
    ...
    def _stop_locked(self) -> None:
        ...
        # fix_a28: 핫키가 실제로 해제될 때까지 기다린다 (최대 1.5초)
        t = self.thread
...
class HistoryManager:
    ...
    def update_latest_message(self, message: str) -> bool:
        """fix_a28: 가장 최근 항목의 message 를 실제 송출 문구로 갱신."""
        with self._lock:
            if not self._items:
                return False
            self._items[0]["message"] = str(message)
            self._save_silent()
            return True
    ...
    def update_latest(self, players: list[dict], chicken_bonus: int,
                      ...
                      final_kill: int, message: str = "",
                      is_chicken: bool | None = None, edited: bool | None = None,
                      edited_from: int | None = None) -> bool:
        ...
        with self._lock:
            ...
            # fix_a28: 치킨 수동 토글·수정 표시
            if is_chicken is not None:
                item["is_chicken"] = bool(is_chicken)
            if edited:
                item["edited"] = True
                if edited_from is not None and "edited_from" not in item:
                    item["edited_from"] = int(edited_from)
            self._save_silent()
            return True
...
class ChzzkChatClient:
        ...
        if token_path.exists():
            try:
                with token_path.open("r", encoding="utf-8") as fp:
                    loaded = json.load(fp)
                with self._lock:
                    self._tokens = loaded if isinstance(loaded, dict) else {}
            except Exception:
                # fix_a28: 읽기 실패 시 메모리 토큰은 지우지 않는다 (파일이 잠깐 깨진 것일 수 있음)
                self.error_logger.exception("토큰 파일 로드 실패: %s (메모리 토큰 유지)", token_path)
        else:
            with self._lock:
                self._tokens = {}

    def save_tokens(self, tokens: dict) -> None:
        # fix_a28: 원자적 쓰기(tmp → os.replace) + 락 안에서 교체.
        # 예전엔 open("w") 직접 덮어쓰기라, 백그라운드 갱신이 쓰는 도중 다른 스레드가 읽으면 JSON 깨짐 → 토큰 소실.
        token_path = self.get_token_path()
    ...
    def _send_message_with_retry(self, message: str) -> str:
        ...
        for attempt in range(max_retries_network + 1):
            try:
                ...
                # fix_a28: 요청은 서버에 닿았을 수 있는데 응답만 못 받은 경우 — 재시도하면 같은 채팅이 두 번 나갈 수 있다.
                self.event_logger.error("치지직 채팅 전송 응답 대기 초과(ReadTimeout) - 중복 방지를 위해 재시도하지 않음: %s", exc)
                raise RuntimeError("치지직 응답이 없어 전송 결과를 확인하지 못했습니다 (중복 방지를 위해 재시도하지 않음)")
            except (requests.ConnectionError, requests.Timeout) as exc:
                # 네트워크 일시 장애(연결 자체 실패) - 재시도
                if attempt < max_retries_network:
                    wait = 0.5 * (2 ** attempt)
                    self.event_logger.warning(
    ...
    def _on_refresh_failure(self, exc: Exception) -> None:
        ...
        # fix_a28: 서버가 4xx 로 거부한 경우(리프레시 토큰 만료·폐기·client_secret 변경)는 다시 시도해도 소용없다.
        # 토큰을 지워서 UI 가 "미연결 / 다시 연결" 로 바뀌게 한다. 네트워크 오류(5xx·연결 실패)는 보존해 재시도.
        status = None
        resp = getattr(exc, "response", None)
        if resp is not None:
            status = getattr(resp, "status_code", None)
        if (status is not None and 400 <= int(status) < 500) or "Refresh Token이 없습니다" in str(exc):
            try:
                self.clear_tokens()
                self.event_logger.error("치지직 리프레시 토큰이 거부됨(%s) → 토큰 삭제. 설정에서 '치지직 연결'을 다시 해주세요.", status)
            except Exception:
                self.error_logger.exception("토큰 무효화 실패")
    ...
    def start_interactive_login(self) -> AuthResult:
        ...
        state = f"state_{secrets.token_urlsafe(16)}"  # fix_a29: 예측 불가 state (예전엔 time.time())
...
def migrate_legacy_data_dir() -> list[str]:
    """fix_a29: a28 이하 설치판은 데이터를 exe 옆(Program Files)에 뒀다. 새 DATA_DIR 에 config.json 이 아직 없고
    예전 위치에 있으면 개인 파일을 한 번 옮겨 온다(복사 — 원본은 남겨 롤백 가능). 옮긴 파일 이름 목록 반환."""
    moved: list[str] = []
    try:
        if DATA_DIR == RUNTIME_BASE_DIR:
            return moved
        if (DATA_DIR / "config.json").exists():
            return moved
        legacy = RUNTIME_BASE_DIR
        if not (legacy / "config.json").exists():
            return moved
        DATA_DIR.mkdir(parents=True, exist_ok=True)
        for name in ["config.json", "config.backup.json", "chat_tokens.json", "history.json"]:
            src = legacy / name
            if src.exists():
                try:
                    ...
                    pass
        # 토큰 파일 이름이 config 로 바뀌어 있을 수 있음
        try:
            cfg = json.loads((legacy / "config.json").read_text(encoding="utf-8"))
            tf = str((cfg.get("chat") or {}).get("token_file", "chat_tokens.json"))
            if tf and not Path(tf).is_absolute() and (legacy / tf).exists() and tf not in moved:
                shutil.copy2(legacy / tf, DATA_DIR / tf)
                moved.append(tf)
        except Exception:
            pass
    except Exception:
...
def bootstrap_runtime_files() -> None:
    ...
    if migrated:
        try:
            LOGS_DIR.mkdir(parents=True, exist_ok=True)
            with (LOGS_DIR / "migration.log").open("a", encoding="utf-8") as fp:
                fp.write(f"[{datetime.now():%Y-%m-%d %H:%M:%S}] {RUNTIME_BASE_DIR} → {DATA_DIR}: {', '.join(migrated)}\n")
        except Exception:
            pass
    for filename in ["config.json", "overlay.txt", "chat_outbox.txt"]:
        runtime_file = DATA_DIR / filename
    ...
    cfg_runtime = DATA_DIR / "config.json"
...
class WebApi:
    ...
    def __init__(self) -> None:
        ...
        self._lock_file = DATA_DIR / ".running.lock"
        ...
        # fix_a28: F8 은 화면 캡처 모드 + 핫키 ON 일 때만, F9 는 항상 (스팀 모드에서도 송출 가능)
        self._apply_hotkey_state("startup")

        self.window = None  # pywebview window, set after creation
        self._drag_lock = threading.Lock()   # fix_a32: 드래그 스레드 중복 방지
        ...
        try:
            ...
            self.event_logger.info("앱 폴더: %s / 데이터 폴더: %s", str(RUNTIME_BASE_DIR), str(DATA_DIR))
    ...
    STEAM_QUEUE_MAX_AGE = 20.0  # fix_a28: 이 시간(초) 넘은 대기열 항목은 버린다 (지난 판 스샷으로 계산 방지)

    def _consume_queued_steam_screenshot(self) -> Path | None:
        """대기열에서 가장 최근 항목 하나를 꺼낸다. 오래된 항목(감지 후 20초 초과)은 버린다.
        fix_a28: 예전엔 계산 중/쿨타임 중에 감지된 스샷이 큐에 남아 있다가
        한참 뒤 '이번 판 계산' 을 누르면 그 지난 판 스샷으로 계산됐다."""
        latest: Path | None = None
        latest_ts = 0.0
        now = time.time()
        while True:
            try:
                item = self._steam_watch_queue.get_nowait()
            except queue.Empty:
                break
            path, ts = item if isinstance(item, tuple) else (item, now)
            if now - ts > self.STEAM_QUEUE_MAX_AGE:
                self.event_logger.info("스팀 대기열 항목 폐기(%.0f초 경과): %s", now - ts, path)
                continue
            if ts >= latest_ts:
                latest, latest_ts = path, ts
        return latest

    def _wait_file_stable(self, target: Path, min_wait: float, max_wait: float = 3.0) -> None:
        """파일 크기가 연속 2회 같아질 때까지 기다린다 (Steam 이 JPG 를 다 쓸 때까지).
        fix_a28: 대기열 경로에서도 이 대기를 거친다. 예전엔 감시 스레드가 파일 생성 직후 큐에 넣고
        계산이 즉시 읽어서, 아직 쓰는 중인 파일을 읽으면 None 이거나 하단이 회색으로 잘린 이미지였다."""
        if min_wait > 0:
            time.sleep(min_wait)
        stable_count = 0
        last_size = -1
        stable_deadline = time.time() + max(0.5, max_wait)
        ...
        try:
            ...
            while not self._steam_watch_stop.wait(0.5):
                try:
                    ...
                    continue
                # fix_a28: 폴더가 일시적으로 비거나(외장/네트워크 드라이브 해제) 못 읽으면 기준 스냅샷을 갱신하지 않는다.
                # 예전엔 빈 스냅샷이 기준이 돼서, 폴더가 돌아오는 순간 모든 파일이 '새 파일' 로 판정 → 과거 스샷으로 자동 전송.
                if not snapshot and current:
                    continue
                new_paths: list[Path] = []
                ...
                latest = new_paths[-1]
                self._steam_watch_queue.put((latest, time.time()))
    ...
    def begin_native_drag(self) -> dict:
        """fix_a32: 타이틀바 mousedown 때 한 번만 호출 → 파이썬 스레드가 마우스 버튼이 눌린 동안 커서 위치를 읽어 SetWindowPos 로 창을 옮긴다.
        a30~a31 의 WM_NCLBUTTONDOWN/HTCAPTION 방식은 실기(2026-08-22, a28~a31 폴더판 비교)에서 창이 전혀 움직이지 않았다.
        JS 핸들러는 도달했고(events.log '[a31] titlebar mousedown') 예외도 없었으므로 Windows 쪽에서 메시지가 무시된 것 —
        ReleaseCapture() 는 "호출한 스레드가 가진 캡처" 만 풀기 때문에 별도 스레드에서는 WebView2 의 캡처를 풀지 못했을 가능성이 가장 크다.
        이 방식은 캡처·메시지 경로와 무관하고, 브리지 왕복(easy_drag) 도 없다."""
        if os.name != "nt" or self.window is None:
            return {"ok": False}
        try:
            hwnd = int(self.window.native.Handle.ToInt32())
        except Exception:
            self.error_logger.exception("네이티브 드래그: hwnd 획득 실패")
            return {"ok": False}
        with self._drag_lock:
            ...
            def _drag() -> None:
                try:
                    ...
                    if not user32.GetCursorPos(ctypes.byref(pt)) or not user32.GetWindowRect(hwnd, ctypes.byref(rc)):
                        return
                    off_x, off_y = rc.left - pt.x, rc.top - pt.y
                    start_x, start_y = rc.left, rc.top
                    SWP_FLAGS = 0x0001 | 0x0004 | 0x0010  # NOSIZE | NOZORDER | NOACTIVATE
                    VK_LBUTTON = 0x01
                    last = (rc.left, rc.top)
                    while user32.GetAsyncKeyState(VK_LBUTTON) & 0x8000:
                        if user32.GetCursorPos(ctypes.byref(pt)):
                            nx, ny = pt.x + off_x, pt.y + off_y
                            if (nx, ny) != last:
                                user32.SetWindowPos(hwnd, 0, nx, ny, 0, 0, SWP_FLAGS)
                                last = (nx, ny)
                        time.sleep(0.006)
                    if last != (start_x, start_y):
                        self.event_logger.info("[a32] 창 이동: (%d,%d) → (%d,%d)", start_x, start_y, last[0], last[1])
                except Exception:
                    self.error_logger.exception("네이티브 드래그 루프 오류")

            self._drag_thread = threading.Thread(target=_drag, name="native-drag", daemon=True)
    ...
    def get_config(self) -> dict:
        ...
        # fix_a28: 예전엔 여기서 매번 load_tokens() 로 파일을 다시 읽었다 (갱신 중 읽으면 토큰 소실). 메모리가 진실.
        ...
        return {
            ...
            "chat_message_format": self.config.get("chat_message_format", "!미션 {합계}"),  # fix_a29: 계산부 기본값과 통일
    ...
    def refresh_monitors(self) -> list[str]:
        # fix_a29: [새로고침] 은 캐시를 무시하고 다시 센다
        self._monitor_list_cache_time = 0
        return self._get_monitor_list()

    # ── fix_a28: 설정은 바꾸는 즉시 저장 (예전엔 보너스만 즉시 저장되고 나머지는 "설정 저장" 전 종료 시 유실) ──
    def _persist(self, what: str = "") -> None:
        try:
            ...
            self.error_logger.exception("설정 즉시 저장 실패 (%s)", what)

    def _apply_hotkey_state(self, reason: str = "") -> None:
        """fix_a28: F8 은 '화면 캡처 모드 + 핫키 ON' 일 때만, F9(송출) 는 항상 등록.
        예전엔 스팀 모드로 바꾸면 HotkeyManager 전체가 내려가 F9 도 같이 죽었다."""
        steam = self.config.get("capture_mode", "screen") == "steam"
        f8 = bool(self._hotkey_enabled) and not steam
        try:
            self._hotkey_manager.start(enable_f8=f8, enable_f9=True)
            self.event_logger.info("핫키 상태 적용(%s): F8=%s F9=%s", reason, f8, True)
        except Exception:
            self.error_logger.exception("핫키 상태 적용 실패 (%s)", reason)

    def set_monitor(self, index: str) -> None:
        self._monitor_var = str(index)
        self._persist("monitor")

    def set_hotkey(self, enabled: bool) -> None:
        self._hotkey_enabled = bool(enabled)
        self._apply_hotkey_state("set_hotkey")
        self._persist("hotkey")

    def set_chat_enabled(self, enabled: bool) -> None:
        self._chat_enabled = bool(enabled)
        self._persist("chat_enabled")
    ...
    def set_capture_mode(self, mode: str) -> None:
        if mode in ("screen", "steam"):
            ...
            # fix_a28: 스팀 모드 = F8 만 내리고 F9 는 유지
            self._apply_hotkey_state("set_capture_mode")
            self._sync_steam_watch_state(reason="set_capture_mode")
            self._persist("capture_mode")
    ...
    def set_steam_screenshot_dir(self, path: str) -> None:
        ...
        self._persist("steam_screenshot_dir")
    ...
    def set_steam_screenshot_delay(self, value: str) -> None:
        try:
            ...
            self._persist("steam_screenshot_delay")
        except Exception:
            pass
    ...
    def _calculate_impl(self) -> dict:
        ...
        try:
            ...
            # 치킨(#1) 감지: 좌상단 순위 글자 기하로 판별 (fix_a28 재설계)
            chicken_state, chicken_info = self._detect_rank_state(frame_bgr)
            self.event_logger.info("치킨 감지: %s (total=%d right=%d %s)",
                                   {True: "✅ 치킨!", False: "❌ 비치킨", None: "❔ 판정 불가"}[chicken_state],
                                   chicken_info.get("total", 0), chicken_info.get("digit_right", -1), chicken_info.get("reason", ""))
            if chicken_state is None and self._last_capture_mode_used == "screen":
                # fix_a28: 결과창 페이드 중이면 순위 글자가 아직 어둡다 → 0.4초 뒤 한 번 다시 찍어서 순위만 다시 본다
                try:
                    time.sleep(0.4)
                    retry_frame = self._grab_frame()
                    if retry_frame is not None:
                        rh, rw = retry_frame.shape[:2]
                        if rw != 1920 or rh != 1080:
                            retry_frame = cv2.resize(retry_frame, (1920, 1080), interpolation=cv2.INTER_LINEAR)
                        chicken_state, chicken_info = self._detect_rank_state(retry_frame)
                        self.event_logger.info("치킨 재감지(0.4초 후): %s (%s)",
                                               {True: "✅ 치킨!", False: "❌ 비치킨", None: "❔ 판정 불가"}[chicken_state],
                                               chicken_info.get("reason", ""))
                        if chicken_state is not None:
                            frame_bgr = retry_frame  # 재캡처본이 더 선명하므로 숫자도 이걸로 읽는다
                except Exception:
                    self.error_logger.exception("치킨 재감지 실패")
            is_chicken = chicken_state is True
            chicken_uncertain = chicken_state is None
            ...
            sanity_cfg = self.config.get("sanity", {}) or {}
            max_damage = int(sanity_cfg.get("max_damage_per_player", 10000))
            # fix_a29: 모드별 현실 상한 (곡천님: 스쿼드에서 혼자 3000딜 이상은 드묾). 9999 같은 오독 합계가 채팅으로 나가는 걸 막는다.
            # 값은 config.sanity.max_damage_per_player_by_mode 로 조절. 초과 칸은 0 처리 + 경고 → 사용자가 [수정] 으로 바로잡는다.
            by_mode = sanity_cfg.get("max_damage_per_player_by_mode", {}) or {}
            try:
                max_damage = min(max_damage, int(by_mode.get(mode, max_damage)))
            except Exception:
                pass
            max_kill = int(sanity_cfg.get("max_kill_per_player", 30))
            details: list[RoiResult] = []
            total_damage = 0
            total_kill = 0
            sanity_warnings: list[str] = []  # fix_a29: 상한 초과로 0 처리된 칸을 UI 에 알린다
            ...
            for idx, player in enumerate(mode_cfg.get("players", []), start=1):
                ...
                if read_target in ("damage", "both"):
                    ...
                    if damage > max_damage:
                        sanity_warnings.append(f"P{idx} 딜 {damage}은 상한({max_damage}) 초과 → 0 처리")
                        damage = 0
                    total_damage += damage
                ...
                if read_target in ("kill", "both"):
                    if "kill" in player:
                        ...
                        if kill > max_kill:
                            sanity_warnings.append(f"P{idx} 킬 {kill}은 상한({max_kill}) 초과 → 0 처리")
                            kill = 0
                        total_kill += kill

                details.append(RoiResult(idx, kill, damage))

            comp = self._compose_result(total_damage, total_kill, is_chicken)
            chicken_bonus = comp["chicken_bonus"]
            kill_bonus = comp["kill_bonus"]
            ...
            final_damage = comp["final_damage"]
            final_kill = comp["final_kill"]
            read_target = comp["read_target"]
            result_text = comp["message"]
            self._last_final_damage = final_damage
            self._last_final_kill = final_kill
            ...
            return {
                ...
                "read_target": read_target,
                "is_chicken": is_chicken,
                "chicken_uncertain": chicken_uncertain,  # fix_a28: 순위를 못 읽음 → UI 가 치킨 토글을 강조
                "chicken_bonus": chicken_bonus,
                "kill_bonus": kill_bonus,
                "multiplier": self._multiplier,
                "final": final_damage,
    ...
    def _grab_from_steam(self, steam_dir: str) -> np.ndarray | None:
        ...
        try:
            ...
            if queued_target is not None and queued_target.exists():
                # fix_a28: 감지 시점부터 delay 만큼 기다린 뒤 크기 안정화까지 확인하고 읽는다
                try:
                    detected_age = max(0.0, time.time() - queued_target.stat().st_mtime)
                except Exception:
                    detected_age = 0.0
                self._wait_file_stable(queued_target, min_wait=max(0.0, base_delay - detected_age))
                img = load_image_unicode_safe(queued_target, cv2.IMREAD_COLOR)
                if img is not None:
                    self._last_capture_source = str(queued_target)
                    self.event_logger.info("스팀 스크린샷 대기열 사용: %s (delay=%.1f초)", queued_target, base_delay)
                    return img
                self.event_logger.warning("대기열 스샷 읽기 실패 → 수동 대기로 전환: %s", queued_target)
            ...
            self._wait_file_stable(target, min_wait=base_delay)
    ...
    def _detect_chicken(self, frame_bgr: np.ndarray) -> bool:
        """(호환용) True/False 만 필요할 때. 판정 불가(None)는 False."""
        return self._detect_rank_state(frame_bgr)[0] is True

    def _detect_rank_state(self, frame_bgr: np.ndarray):
        """fix_a28: 좌상단 순위 표시("#1/27")의 노란 글자 기하로 치킨 판정.
        반환 (is_chicken, info)  — is_chicken: True / False / None(순위가 아직 안 그려짐·판정 불가)

        a25 까지는 '노란 픽셀 개수 > 500 이고 x=110~125 구간 픽셀 < 100' 이었다. 문제 두 가지(실측):
          ① 결과창 페이드 중(글자가 어두움) 에는 픽셀 수가 모자라 치킨을 놓침 → "+1000 만 빠짐"
          ② 페이드 중 + 채팅 오버레이가 겹치면 '#20' 의 0 이 끊겨 치킨으로 오판 (회귀 캡처 20260421050652 실물)
        이제는 (a) 채도/명도 문턱을 낮추고 (b) '#' 글자 뒤 첫 숫자 덩어리의 오른쪽 끝 x 좌표로 판정한다.
        1080p 정규화 기준 실측(QHD 102장 + FHD 53장): '#' = x34~78, '1' = x84~111, '2'~'9' = x85~117, 두 자리면 x118 이후에 덩어리 추가.
        '#' 이 안 보이면(total=0 등) None 을 돌려주고, 호출부가 화면 캡처 모드면 0.4초 뒤 한 번 다시 찍는다.
        """
        info = {"total": 0, "segs": [], "digit_right": -1, "reason": ""}
        try:
            roi = frame_bgr[40:160, 40:300]
            hsv = cv2.cvtColor(roi, cv2.COLOR_BGR2HSV)
            mask = cv2.inRange(hsv, np.array([18, 80, 90]), np.array([38, 255, 255]))
            total_px = int(mask.sum() // 255)
            info["total"] = total_px
            col = (mask > 0).sum(axis=0)
            cols = np.where(col >= 2)[0]
            if len(cols) == 0:
                info["reason"] = "노란 글자 없음(순위 미표시)"
                return None, info
            segs = []
            st = prev = int(cols[0])
            for c in cols[1:]:
            ...
            segs.append((st, prev))
            info["segs"] = segs
            # '#' 덩어리: 왼쪽 끝 28~42, 오른쪽 끝 70~82
            hash_seg = next((sg for sg in segs if 28 <= sg[0] <= 42 and 70 <= sg[1] <= 82), None)
            if hash_seg is None:
                # '#7' 처럼 7 의 윗줄이 '#' 에 붙어 한 덩어리(x34~115)로 잡히는 경우 — 1 은 절대 붙지 않으므로 비치킨
                merged = next((sg for sg in segs if 28 <= sg[0] <= 42 and sg[1] >= 100), None)
                if merged is not None:
                    info["digit_right"] = merged[1]
                    info["reason"] = "'#'+숫자 붙음(7 등) → 비치킨"
                    return False, info
                info["reason"] = "'#' 미검출(페이드/가림)"
                return None, info
            digits = [sg for sg in segs if sg[0] > hash_seg[1]]
            if not digits:
                info["reason"] = "'#' 뒤 숫자 없음"
                return None, info
            first = digits[0]
            info["digit_right"] = first[1]
            if len(digits) >= 2:
                info["reason"] = "두 자리 순위"
                return False, info
            if first[1] <= 113:
                info["reason"] = "한 자리·좁음 → #1"
                return True, info
            if first[1] >= 116:
                info["reason"] = "한 자리·넓음 → #2~#9"
                return False, info
            info["reason"] = "경계값(114~115) 판정 보류"
            return None, info
        except Exception as exc:
            info["reason"] = f"예외 {type(exc).__name__}"
            self.error_logger.exception("치킨 감지 예외")
            return None, info
    ...
    SEND_MIN_INTERVAL = 1.0  # fix_a28: 연속 전송 최소 간격(초) — F9 연타/자동+수동 겹침 보호

    def _send_chat(self, message: str, mark_history_id: int | None = None, status_label: str = "전송") -> bool:
        """fix_a28: 전송을 직렬화하고, 결과를 UI(_last_send_status)와 기록(sent 플래그)에 반영한다.
        예전엔 스레드만 띄우고 곧바로 '전송 완료' 로 표시해 실패가 errors.log 에만 남았다.
        mark_history_id: 성공 시 sent=True 로 표시할 기록 id (None 이면 가장 최근 항목)."""
        with self._send_lock:
            gap = time.time() - self._last_send_ts
            if gap < self.SEND_MIN_INTERVAL:
                time.sleep(self.SEND_MIN_INTERVAL - gap)
            try:
                self.chat_client.send_message(message)
                self._last_send_ts = time.time()
                self.event_logger.info("치지직 %s 완료: %s", status_label, message)
                try:
                    if mark_history_id is None:
                        self.history.mark_sent_latest()
                    else:
                        self.history.mark_sent_by_id(int(mark_history_id))
                except Exception:
                    self.error_logger.exception("history sent 표시 실패")
                self._last_send_status = f"{status_label} 완료"
                return True
            except Exception as exc:
                self.error_logger.exception("치지직 %s 실패", status_label)
                self._last_send_status = f"{status_label} 실패: {str(exc)[:60]}"
                return False

    # ── fix_a28: 보너스·배수·메시지 조립을 한 곳으로 (계산/F9/수정/치킨 토글 네 경로가 같은 식을 쓰도록) ──
    def _compose_result(self, total_damage: int, total_kill: int, is_chicken: bool) -> dict:
        read_target = self.config.get("read_target", "damage")
        read_damage = read_target in ("damage", "both")
        read_kill = read_target in ("kill", "both")
        chicken_bonus = 0
        kill_bonus = 0
        # fix_a28: 읽지 않은 대상에는 보너스를 더하지 않는다
        # (예전엔 킬 모드에서 딜을 안 읽었는데도 {딜합계} 가 "1000"(보너스만) 으로 나갔다)
        if is_chicken and read_damage and self.config.get("chicken_bonus_enabled", False):
            chicken_bonus = int(self.config.get("chicken_bonus_damage", 0))
        if is_chicken and read_kill and self.config.get("kill_bonus_enabled", False):
            kill_bonus = int(self.config.get("kill_bonus_count", 0))
        final_damage = (int(total_damage) + chicken_bonus) * self._multiplier
        final_kill = (int(total_kill) + kill_bonus) * self._multiplier
        ...
        return {
            "read_target": read_target, "chicken_bonus": chicken_bonus, "kill_bonus": kill_bonus,
            "final_damage": final_damage, "final_kill": final_kill, "message": message,
            "multiplier": self._multiplier,
        }

    def get_last_result(self) -> dict:
        """fix_a28: 메인 카드 [수정] 패널용 — 현재 결과와 수정 가능 여부."""
        try:
            items = self.history.list_all()
            latest = items[0] if items else None
            details = list(getattr(self, "_last_details", []) or [])
            editable = self.history.is_session_dirty() and bool(details)
            return {
                "editable": editable,
                "reason": "" if editable else "이번 실행에서 F8로 읽은 결과가 아직 없어요.",
                "players": [{"kill": d.kill, "damage": d.damage} for d in details],
                "read_target": self.config.get("read_target", "damage"),
                "is_chicken": bool(getattr(self, "_last_is_chicken", False)),
                "sent": bool(latest and latest.get("sent")),
                "edited": bool(latest and latest.get("edited")),
                "final": int(latest.get("final", 0)) if latest else 0,
                "final_kill": int(latest.get("final_kill", 0)) if latest else 0,
            }
        except Exception as exc:
            self.error_logger.exception("get_last_result 실패")
            return {"editable": False, "reason": str(exc), "players": []}

    def _apply_recompute(self, new_details: list, is_chicken: bool, edited: bool, note: str) -> dict:
        """현재 결과(_last_*)와 최근 기록을 새 값으로 갱신하고 UI 용 dict 반환."""
        total_damage = sum(d.damage for d in new_details)
        total_kill = sum(d.kill for d in new_details)
        prev_final = int(getattr(self, "_last_final_damage", 0) or 0)
        prev_final_kill = int(getattr(self, "_last_final_kill", 0) or 0)
        comp = self._compose_result(total_damage, total_kill, is_chicken)
        self._last_details = new_details
        self._last_damage = total_damage
        self._last_kill = total_kill
        self._last_is_chicken = bool(is_chicken)
        self._last_final_damage = comp["final_damage"]
        self._last_final_kill = comp["final_kill"]
        self._last_result_text = comp["message"]
        if self._chat_enabled and self._pending_auto_send_message:
            self._pending_auto_send_message = comp["message"]
        try:
            OVERLAY_PATH.write_text(comp["message"], encoding="utf-8")
            OUTBOX_PATH.write_text(comp["message"], encoding="utf-8")
        except Exception:
            pass
        players_list = [{"kill": d.kill, "damage": d.damage} for d in new_details]
        try:
            self.history.update_latest(
                players=players_list, chicken_bonus=comp["chicken_bonus"], kill_bonus=comp["kill_bonus"],
                multiplier=self._multiplier, total_damage=total_damage, total_kill=total_kill,
                final=comp["final_damage"], final_kill=comp["final_kill"], message=comp["message"],
                is_chicken=bool(is_chicken), edited=edited,
                edited_from=(prev_final if comp["read_target"] != "kill" else prev_final_kill) if edited else None,
            )
        except Exception:
            self.error_logger.exception("history update_latest 실패 (%s)", note)
        self.event_logger.info("%s: 딜합=%d 킬합=%d 치킨=%s 최종=%d | %s", note, total_damage, total_kill,
                               is_chicken, comp["final_damage"], comp["message"])
        return {
            "ok": True,
            "p_damage": [d.damage for d in new_details],
            "p_kill": [d.kill for d in new_details],
            "total_damage": total_damage, "total_kill": total_kill,
            "final": comp["final_damage"], "final_kill": comp["final_kill"],
            "read_target": comp["read_target"], "is_chicken": bool(is_chicken),
            "chicken_bonus": comp["chicken_bonus"], "kill_bonus": comp["kill_bonus"],
            "multiplier": self._multiplier, "message": comp["message"],
        }

    def apply_edit(self, players, send: bool = False) -> dict:
        """fix_a28: [수정] 패널의 [적용]. players = [{kill, damage}, ...] (없는 키는 기존값 유지).
        더블클릭 편집(a19~a20)을 버튼 방식으로 교체. 이미 송출된 결과도 수정 가능하며,
        send=True 면 수정된 값의 메시지를 한 번 더 보낸다 (곡천님 결정: 수정 양식 2회 전송은 음수 테스트 후)."""
        try:
            if not self.history.is_session_dirty() or not getattr(self, "_last_details", None):
                return {"ok": False, "error": "이번 실행에서 F8로 읽은 결과가 아직 없어요."}
            if not hasattr(players, "__iter__") or isinstance(players, (str, bytes, dict)):
                return {"ok": False, "error": "인자 형식 오류"}
            cur = list(self._last_details)
            new_details: list[RoiResult] = []
            changed = False
            for idx, d in enumerate(cur):
                k, dm = d.kill, d.damage
                if idx < len(players) and isinstance(players[idx], dict):
                    pk = players[idx].get("kill", None)
                    pd = players[idx].get("damage", None)
                    if pk is not None and str(pk).strip() != "":
                        k = max(0, int(float(pk)))
                    if pd is not None and str(pd).strip() != "":
                        dm = max(0, int(float(pd)))
                if (k, dm) != (d.kill, d.damage):
                    changed = True
                new_details.append(RoiResult(idx + 1, k, dm))
            res = self._apply_recompute(new_details, bool(getattr(self, "_last_is_chicken", False)),
                                        edited=changed, note="카드 수정 적용")
            if send:
                msg = res["message"]
                threading.Thread(target=self._send_chat, args=(msg, None, "수정본 전송"), daemon=True).start()
                res["status"] = "수정본 전송 중..."
            return res
        except Exception as exc:
            self.error_logger.exception("apply_edit 실패")
            return {"ok": False, "error": str(exc)}

    def set_last_is_chicken(self, flag: bool) -> dict:
        """fix_a28: 메인 카드의 치킨 수동 토글 — 감지가 틀렸을 때 사용자가 직접 켜고 끈다."""
        try:
            if not self.history.is_session_dirty() or not getattr(self, "_last_details", None):
                return {"ok": False, "error": "이번 실행에서 F8로 읽은 결과가 아직 없어요."}
            res = self._apply_recompute(list(self._last_details), bool(flag), edited=True, note="치킨 수동 토글")
            return res
        except Exception as exc:
            self.error_logger.exception("set_last_is_chicken 실패")
            return {"ok": False, "error": str(exc)}
        ...
        try:
            ...
            if self._last_damage == 0 and self._last_kill == 0 and not self._last_details:
                if from_hotkey:
                    self._last_send_status = "전송할 결과가 없습니다"
                return {"ok": False, "status": "전송할 결과가 없습니다"}
            comp = self._compose_result(self._last_damage, self._last_kill, bool(getattr(self, '_last_is_chicken', False)))
            msg = comp["message"]
            # fix_a28: 기록의 message 도 송출 시점 값으로 맞춘다 (F8 뒤 설정을 바꾼 경우 기록과 실제 송출이 달랐음)
            try:
                self.history.update_latest_message(msg)
            except Exception:
                pass
            # fix_a28: sent 표시·완료 문구는 실제 전송 결과가 나온 뒤에 (_send_chat 안에서)
            threading.Thread(target=self._send_chat, args=(msg, None, "전송"), daemon=True).start()
            return {"ok": True, "status": "전송 중...", "message": msg}
        except Exception:
    ...
    def dispatch_auto_send(self) -> dict:
        ...
        # fix_a28: sent 표시는 실제 전송 성공 뒤에 (_send_chat 안에서). 실패하면 기록은 '미송출' 로 남아 재전송 가능.
        threading.Thread(target=self._send_chat, args=(message, None, "자동 전송"), daemon=True).start()
    ...
    def update_last_result(self, p_kills, p_damages) -> dict:
        """(호환용) a19~a20 더블클릭 편집 API. fix_a28 부터는 apply_edit 로 위임.
        예전 구현의 문제: ① 자동 전송 ON 이면 '이미 송출됨' 으로 항상 거부 ② 딜만 고쳐도 킬을 0 으로 덮어씀."""
        try:
            kills = list(p_kills) if hasattr(p_kills, "__iter__") and not isinstance(p_kills, (str, bytes, dict)) else []
            dmgs = list(p_damages) if hasattr(p_damages, "__iter__") and not isinstance(p_damages, (str, bytes, dict)) else []
            n = max(len(kills), len(dmgs))
            players = [{"kill": kills[i] if i < len(kills) else None,
                        "damage": dmgs[i] if i < len(dmgs) else None} for i in range(n)]
            return self.apply_edit(players, send=False)
        except Exception as exc:
            return {"ok": False, "error": str(exc)}
    ...
    def resend_history(self, history_id: int) -> dict:
        ...
        try:
            ...
            # fix_a28: sent 표시는 전송 성공 뒤에 (_send_chat 안에서 id 로 표시)
            threading.Thread(target=self._send_chat, args=(message, int(history_id), "재전송"), daemon=True).start()
            self.event_logger.info("기록 재전송 시작: id=%d message=%s", history_id, message)
            return {"ok": True, "message": message, "status": "재전송 중..."}
        except Exception as exc:
    ...
    def start_update(self) -> dict:
        ...
        def _worker() -> None:
            try:
                ...
                path = updater.download_installer(info["url"], _progress, info.get("sha256_url", ""))
    ...
    def open_folder(self, name: str) -> None:
        mapping = {"screenshot": SCREENSHOTS_DIR, "debug": DEBUG_DIR, "logs": LOGS_DIR, "config": DATA_DIR, "data": DATA_DIR}
...
def main() -> None:
    ...
    window = webview.create_window(
        ...
        background_color="#141820",
        frameless=True,
        easy_drag=False,   # fix_a30: JS→파이썬 왕복으로 창을 끌던 easy_drag 끔 (드래그 지연·브리지 정체의 원인)
        resizable=False,
        shadow=True,
    )
    api.window = window
    ...
    def on_shown():
        # fix_a30: DWM 둥근 모서리 훅을 before_show → shown 으로 이동 (v5 진단: before_show 의 동기 DWM 호출과 창 생성이 겹치면 멈춤 의심)
        try:
            _apply_windows_rounded_corners(window, api)
        except Exception:
            api.error_logger.exception("둥근 모서리 적용 실패")
