...
class WebApi:
    ...
    def begin_native_drag(self) -> dict:
        """fix_a30: 타이틀바 mousedown 때 한 번만 호출 → Windows 가 창 이동을 직접 처리한다 (WM_NCLBUTTONDOWN/HTCAPTION).
        예전(easy_drag) 은 마우스가 움직일 때마다 JS→파이썬 호출로 창을 옮겨서, 브리지가 조금만 느려도 창이 뒤늦게 따라오고
        다른 버튼 호출까지 그 뒤에 줄을 섰다 (2026-08-20 a29 실기 영상)."""
        ...
        try:
            ...
            def _drag() -> None:
                # SendMessage 는 드래그가 끝날 때까지 돌아오지 않으므로 별도 스레드에서 (브리지 스레드를 잡아두지 않게)
                user32 = ctypes.windll.user32
            ...
            threading.Thread(target=_drag, name="native-drag", daemon=True).start()
            return {"ok": True}
        except Exception:
            self.error_logger.exception("네이티브 드래그 시작 실패")
            return {"ok": False}
