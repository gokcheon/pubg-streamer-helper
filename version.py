...
태그 규칙: 올리기.bat 이 APP_LABEL(fix_a27)을 그대로 git 태그로 쓴다.
updater.tag_fix() 는 태그 문자열의 마지막 숫자(26)만 본다.
"""
APP_FIX = 27                          # fix 번호 (자동 업데이트 비교 기준)
...
if __name__ == "__main__":
    ...
    # `python version.py label` → fix_a27  (올리기.bat 이 릴리스 태그로)
    import sys
